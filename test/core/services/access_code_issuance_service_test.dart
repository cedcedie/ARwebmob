...
void main() {
  test('issueSubjectCode (whole-subject, no lessonIds) round-trips through redeem() for an untargeted student', () async {
    ...
    final result = await accessCodeService.redeem(studentId: '111111', rawCode: code);

    expect(result.success, true);
    expect(result.message, 'Subject unlocked successfully!');
  });

  test('issueSubjectCode with lessonIds: redeem() succeeds for a listed lesson, fails for one not in the list', () async {
    final firestore = FakeFirebaseFirestore();
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('111111'));
    final issuance = AccessCodeIssuanceService(
      firestore: firestore,
  ...
  test('issueLessonCode: redeem() succeeds for the targeted student, fails "different student" for another', () async {
    final firestore = FakeFirebaseFirestore();
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('111111'));
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('222222'));
    final issuance = AccessCodeIssuanceService(
      firestore: firestore,
      quizAttemptService: QuizAttemptService(firestore: firestore),
    );
    ...
    final code = await issuance.issueLessonCode(lessonId: 'q2w2', studentId: '111111');
  ...
  test('issueQuizRetakeCode throws and writes nothing when the student has zero attempts on the post-test', () async {
    final firestore = FakeFirebaseFirestore();
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('111111'));
    final issuance = AccessCodeIssuanceService(
      firestore: firestore,
  ...
  test(
    ...
    () async {
      ...
      final code = await issuance.issueQuizRetakeCode(lessonId: 'q1w1', studentId: '111111');
  ...
  test('a duplicate teacher-supplied custom code is rejected with no overwrite of the existing doc', () async {
    final firestore = FakeFirebaseFirestore();
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('111111'));
    final issuance = AccessCodeIssuanceService(
      firestore: firestore,
    ...
    await issuance.issueSubjectCode(subjects: ['chemistry'], customCode: 'MYCODE1');

    await expectLater(
      () => issuance.issueSubjectCode(subjects: ['biology'], customCode: 'mycode1'),
      throwsStateError,
    );

    final doc = await firestore.collection('unlockCodes').doc('MYCODE1').get();
    expect(doc.data()!['subjects'], ['chemistry']);
  });

  test('watchIssuedUnlockCodes streams the docs written by issueSubjectCode and issueLessonCode', () async {
    final firestore = FakeFirebaseFirestore();
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('111111'));
    final issuance = AccessCodeIssuanceService(
      firestore: firestore,
    ...
    await issuance.issueSubjectCode(subjects: ['chemistry'], customCode: 'STREAM01');
    await issuance.issueLessonCode(lessonId: 'q1w1', studentId: '111111', customCode: 'STREAM02');
  ...
  test('watchIssuedRetakeCodes streams QuizUnlockCode models written by issueQuizRetakeCode', () async {
    final firestore = FakeFirebaseFirestore();
    final quizAttemptService = QuizAttemptService(firestore: firestore);
    await StudentRepository(firestore: firestore).saveStudent(_blankStudent('111111'));
    final postQuizId = builtinQuizId('q1w1', QuizPhase.post);
