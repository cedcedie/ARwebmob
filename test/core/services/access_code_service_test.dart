...
void main() {
  ...
  test(
    'a full-subject code succeeds from the generic Home entry (no target)',
    () async {
      final firestore = FakeFirebaseFirestore();
  ...
  test(
    'a subject code with a lesson-id list only unlocks listed lessons for a matching target',
    () async {
      final firestore = FakeFirebaseFirestore();
      await StudentRepository(
        firestore: firestore,
  ...
  test(
    'a quiz-retake code unlocks a locked post-test for exactly one more attempt',
    () async {
      final firestore = FakeFirebaseFirestore();
  ...
  test(
    'a first-time test-unlock code (type lesson, redeemed against a quiz) succeeds',
    () async {
      final firestore = FakeFirebaseFirestore();
      await StudentRepository(
        firestore: firestore,
  ...
  test(
    'a manually-created quiz-retake code (type quiz in /unlockCodes) unlocks a locked post-test',
    () async {
      final firestore = FakeFirebaseFirestore();
      final studentRepo = StudentRepository(firestore: firestore);
      final quizAttemptService = QuizAttemptService(firestore: firestore);
  ...
  test(
    'a code assigned to a different student is rejected, echoing the typed code',
    () async {
      final firestore = FakeFirebaseFirestore();
      await StudentRepository(
        firestore: firestore,
  ...
  test(
    'a code already used by this student cannot be redeemed a second time',
    () async {
      final firestore = FakeFirebaseFirestore();
      await StudentRepository(
        firestore: firestore,
