...
QuizAttempt _attempt(String studentId, int score, List<int> answers) =>
    QuizAttempt(
...
void main() {
  test(
    'difficulty index is the fraction of students correct on each question',
    () {
      final attempts = [
      ...
      final result = computeItemAnalysis(
        questions: _twoQuestions(),
        attempts: attempts,
      );

      expect(
        result[0].difficultyIndex,
        closeTo(2 / 4, 0.0001),
      ); // q0: students 1,2 correct
      expect(
        result[1].difficultyIndex,
        closeTo(2 / 4, 0.0001),
      ); // q1: students 1,3 correct
    },
  );
  ...
  test('distractor rates report the fraction choosing each wrong option', () {
    ...
    expect(
      result[0].distractorRates.containsKey(0),
      false,
    ); // correct option isn't a distractor
  });
