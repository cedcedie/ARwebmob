...
class _TrackingAuthService extends AuthService {
  _TrackingAuthService({this.onSignInStudent})
...
void main() {
  group('StudentAuthViewModel', () {
    test(
      'submit with a dash-formatted id calls signInStudent with that value',
      () async {
    ...
    test(
      'submit with a literal email calls signInStudent with that email',
      () async {
        final authService = _TrackingAuthService();
        final vm = StudentAuthViewModel(authService: authService)
    ...
    test(
      'submit with an empty id shows an error without calling AuthService',
      () async {
        final authService = _TrackingAuthService();
        final vm = StudentAuthViewModel(authService: authService)
          ..idOrEmail = '   '
    ...
    test(
      ...
      () async {
        final authService = _TrackingAuthService(
          onSignInStudent:
              ({required String idOrEmail, required String password}) async {
    ...
    test(
      ...
      () async {
        ...
        expect(authService.signInStudentCallCount, 1);
        expect(
          authService.signOutCallCount,
          1,
          reason: 'archived student must be signed back out',
        );
    ...
    test(
      'archived student is signed back out via the literal-email input path too',
      () async {
        // The digit-id path is covered above. The archive check derives the
        // student id from Firebase's returned user.email rather than the raw
        // input, so both input paths (raw digit id, literal email) should
        // converge on the same check — this locks that in directly instead of
        // leaving it to be inferred from reading the code.
        final firestore = FakeFirebaseFirestore();
        final studentRepository = StudentRepository(firestore: firestore);
        await studentRepository.saveStudent(
          _studentRecord(id: '123456', isArchived: true),
        ...
        // Unlike the default _TrackingAuthService stub (which always appends
        // @arscience.school, correct only for the raw-digit-id path), the
        // real AuthService.signInStudent returns the literal email as-is when
        // one is typed — mirror that here so this test exercises the same
        // "already a full email" shape the archive check has to handle.
        final authService = _TrackingAuthService(
          onSignInStudent:
              ({required String idOrEmail, required String password}) async {
                return MockUser(uid: 'uid-student', email: idOrEmail);
              },
        );
        final vm =
            StudentAuthViewModel(
        ...
        expect(authService.signInStudentCallCount, 1);
        expect(authService.lastIdOrEmail, '123456@arscience.school');
        expect(
          authService.signOutCallCount,
          1,
          reason: 'archived student must be signed back out',
    ...
    test(
      'non-archived student sign-in is unaffected by the archive check',
      () async {
        final firestore = FakeFirebaseFirestore();
        final studentRepository = StudentRepository(firestore: firestore);
        await studentRepository.saveStudent(
          _studentRecord(id: '123456', isArchived: false),
        );
