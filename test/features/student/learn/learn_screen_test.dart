...
void main() {
  ...
  testWidgets('tapping a locked lesson card opens the access-code sheet (C2)', (tester) async {
    final viewModel = _viewModel(
      activeSubject: SubjectKey.chemistry,
      cards: [
        LessonCardData(
    ...
    await tester.tap(find.text('Planning and Recording Scientific Investigations'));
    await tester.pumpAndSettle();

    expect(find.text('Apply Code'), findsOneWidget);
  });

  testWidgets('switching the subject tab notifies onSelectSubject with the new subject (C1)',
      (tester) async {
    SubjectKey? selected;
    final viewModel = _viewModel(
      activeSubject: SubjectKey.chemistry,
