...
void main() {
  ...
  test(
      ...
      'session, not the stale completed one (the retake dead-end bug)', () async {
    final firestore = FakeFirebaseFirestore();
    final studentRepo = StudentRepository(firestore: firestore);
    final quizAttemptService = QuizAttemptService(firestore: firestore);
    await studentRepo.saveStudent(StudentRecord(
    ...
    container.invalidate(quizSessionControllerProvider(QuizSessionKey.identity(
      studentId: '111111',
      quizId: quizId,
      quizAttemptService: quizAttemptService,
    )));
  ...
  test(
      ...
      'sessions being invalidated', () async {
    final firestore = FakeFirebaseFirestore();
    final quizAttemptService = QuizAttemptService(firestore: firestore);
    final quizId = builtinQuizId('q1w1', QuizPhase.post);
    final container = ProviderContainer();
