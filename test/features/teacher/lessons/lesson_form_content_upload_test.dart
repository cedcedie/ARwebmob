...
void main() {
  testWidgets('uploading a .pptx sets contentStatus to processing on submit', (
    tester,
  ) async {
    ...
    expect(submitted!.contentImageUrls, [
      'https://fake-storage.example/slides.pptx',
    ]);
  });

  testWidgets(
    'uses the SAME lesson id for the Storage upload and the submitted new lesson',
    (tester) async {
  ...
  testWidgets(
    ...
    'when the Cloud Function finishes converting before Save is clicked',
    (tester) async {
          ...
          home: Scaffold(
            body: LessonForm(
              ...
              refetchLesson: (lessonId) async => existing.copyWith(
                contentStatus: 'ready',
                contentImageUrls: const [
                  'https://fake-storage.example/slide-1.png',
                ],
              ),
            ),
          ),
