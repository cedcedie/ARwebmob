...
void main() {
  test('pre-test: retakeable with no access code, ever — 3 consecutive '
      'attempts with zero gating and no code involved at any point', () async {
    ...
    final finalEligibility = await quizAttemptService.checkEligibility(
      '111111',
      preQuizId,
    );
  ...
  test('post-test: first attempt is free; a retake without a code is rejected; '
      'a retake with a real teacher-issued code succeeds', () async {
    final firestore = FakeFirebaseFirestore();
    final studentRepo = StudentRepository(firestore: firestore);
    final quizAttemptService = QuizAttemptService(firestore: firestore);
    ...
    final firstEligibility = await quizAttemptService.checkEligibility(
      '222222',
      postQuizId,
    );
    ...
    final lockedEligibility = await quizAttemptService.checkEligibility(
      '222222',
      postQuizId,
    );
    ...
    final stillLocked = await quizAttemptService.checkEligibility(
      '222222',
      postQuizId,
    );
    ...
    final unlockedEligibility = await quizAttemptService.checkEligibility(
      '222222',
      postQuizId,
    );
    ...
    final afterRetake = await quizAttemptService.checkEligibility(
      '222222',
      postQuizId,
    );
    expect(
      afterRetake.canTake,
      false,
      reason: 'the retake itself locks again on submission',
    );
    expect(afterRetake.attemptCount, 2);
  });

  test('the two rules are not conflated: a pre-test and a post-test for the '
      'same lesson gate independently even when interleaved', () async {
    final firestore = FakeFirebaseFirestore();
    final studentRepo = StudentRepository(firestore: firestore);
    final quizAttemptService = QuizAttemptService(firestore: firestore);
    ...
    final preEligibility = await quizAttemptService.checkEligibility(
      '333333',
      preQuizId,
    );
    ...
    final postEligibilityBeforeAttempt = await quizAttemptService
        .checkEligibility('333333', postQuizId);
    ...
    final postEligibilityAfterAttempt = await quizAttemptService
        .checkEligibility('333333', postQuizId);
    expect(postEligibilityAfterAttempt.canTake, false);

    // ...while the pre-test remains completely unaffected and open.
    final preEligibilityAfterPostAttempt = await quizAttemptService
        .checkEligibility('333333', preQuizId);
